...
document.write(`Requisições com o assunto: Sugestão <br><br>`) // Área que precisa ser alterada pra exibição correta do assunto sendo tratado

for (let i = 0; i < contatos.length; i++){
  if (contatos[i].assunto === "Sugestão"){ //Área que precisa ser alterada para exibir outras opções (Dúvida, sugestão e suporte técnico)
    document.write(`Assunto: ${contatos[i].assunto} | Nome: ${contatos[i].nome} | Email: ${contatos[i].email}  <br>`)
  }
}

let quantidade = 0;
for (let i = 0; i < contatos.length; i++) {
    if (contatos[i].assunto === "Sugestão")  quantidade++ //Área que precisa ser alterada para exibir outras opções (Dúvida, sugestão e suporte técnico)
}
document.write(`<br> Há ${quantidade} ocorrências com esse assunto nas requisições!`)
